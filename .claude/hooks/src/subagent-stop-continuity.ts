...
import { completeAgent, getActiveAgents, cleanupOldAgents, formatAgentSummary } from './agent-tracker.js';
import { notifyAgentComplete } from './notifications.js';
import { recordOutcome } from './smart-suggestions.js';
import { autoExtractLearning } from './learning-engine.js';
...
async function main() {
  ...
  try {
    ...
    // ─────────────────────────────────────────────────────────────────
    // AGENT TRACKER INTEGRATION - Complete the agent and get metrics
    // ─────────────────────────────────────────────────────────────────
    let metricsMessage = '';
    const activeAgents = getActiveAgents(projectDir);
    
    // Find matching agent by name (most recent one with this name)
    const matchingAgent = activeAgents
      .filter(a => a.name === agentInfo.agentName)
    ...
    if (matchingAgent) {
      ...
      if (completed) {
        ...
        // NOTIFICATIONS - Send system notification
        notifyAgentComplete(projectDir, completed.name, true, {
          toolUses: completed.tool_uses,
          tokensK: Math.round(completed.tokens_used / 1000),
          durationS: Math.round((Date.now() - new Date(completed.started_at).getTime()) / 1000)
        });
        
        // SMART SUGGESTIONS - Record outcome for learning
        recordOutcome(projectDir, completed.name, 'success', matchingAgent.task || '');
        
        // LEARNING ENGINE - Auto-extract learnings from this session
        autoExtractLearning(
          projectDir,
          completed.name,
          matchingAgent.task || '',
